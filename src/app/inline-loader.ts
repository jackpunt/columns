...
export class ImageLoader {
  ...
  static defaultSize: XY = { x: 60, y: 60 };
  /**
  ...
  loadImage(fname0: string, ext = this.ext, inline = this.inline): Promise<HTMLImageElement> {
    ...
    const inlineRes = (res: (value: HTMLImageElement | PromiseLike<HTMLImageElement>) => void, rej: (reason?: any) => void) => {
      // Convert path/filename structure to match the imageId pattern
      // (e.g., 'assets/images/meeple-shape.png' -> 'assets_images_meeple-shape_png')
      ...
      if (img) {
        ...
        rej(`failed to load asset from registry. Element ID not found: ${imageId}`);
      }
    }
...
export class AliasLoader {
   ...
   * @param size [ImageLoader.defaultSize] bitmap.scaleXY = size / max(img.width, img.height)
   ...
   */
  getBitmap(name: string, size: number | XY = ImageLoader.defaultSize, offsetReg = true ) {
